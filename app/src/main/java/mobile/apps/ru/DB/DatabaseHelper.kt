...
class DatabaseHelper(context: Context) :
    ...
    companion object {
        private const val DB_NAME = "personals.db"
        private var DB_PATH = ""
    ...
    init {
        if (Build.VERSION.SDK_INT >= 17) DB_PATH =
            context.getApplicationInfo().dataDir.toString() + "/databases/" else DB_PATH =
            "/data/data/" + context.getPackageName().toString() + "/databases/"
        mContext = context
        copyDataBase()
        this.readableDatabase
    }
